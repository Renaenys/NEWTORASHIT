  ...
  export default function CalendarWidget({ fullPage = false }) {
    ...
    // 1️⃣ Merge all sources into a single array
    const items = useMemo(() => {
      ...
      return [...evts, ...meets, ...tasks];
    }, [data.events, data.meetings, data.tasks]);
    ...
    // 4️⃣ Calendar controls
    const api = () => calendarRef.current.getApi();
    const prev = () => api().prev();
    const next = () => api().next();
    const today = () => api().today();

    // 5️⃣ Event click → details + delete
    const onEventClick = ({ event }) => {
      const route = event.extendedProps.type;
      Swal.fire({
        title: event.title,
        html: `
        ...
        >
          ...
          >
            <FullCalendar
              ...
              headerToolbar={false}
              events={filtered} // ← let FC handle updates
